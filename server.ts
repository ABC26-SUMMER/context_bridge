...
export function promptFor(
  ...
  const approved = contexts
    ...
    .join('\n');
  const contextUseContract = contexts.length
    ? contexts.map((context) => {
        const source = `[${context.category}] ${context.title}: ${context.content}`;
        return `- ${source} → 이번 질문에서 이 정보가 답의 선택지, 우선순위, 설명 방식, 실행 난이도 중 무엇을 바꾸는지 판단하세요. 관련성이 있으면 그 변화가 답변에 보이게 하세요.`;
      }).join('\n')
    : '';
  return approved
    ? `${historyBlock}[사용자 질문]\n${query}\n\n[사용자가 승인한 맥락]\n${approved}\n\n` +
        `[개인화 답변 계약]\n` +
        `승인된 프로필은 장식이 아니라 답변을 바꾸는 입력입니다.\n` +
        `${contextUseContract}\n\n` +
        `[답변 설계 지시]\n` +
        ...
        `5. 승인된 맥락이 답을 구체화할 수 있는데도 일반론으로만 답하지 마세요. 단, 모르는 사실을 꾸며내지 말고 확인이 필요한 부분은 분리하세요.\n` +
        `6. 정보가 부족하면 추측하지 말고 답변 가능한 범위와 확인할 질문을 분리하세요.\n` +
        `7. 출력은 질문에 맞게 '추천/결론 → 이유 → 실행 단계 → 주의점' 순으로 구성하세요.\n` +
        `8. 끝에 '✨ 반영된 맥락' 섹션을 만들고, 실제 답변을 어떻게 바꾼 맥락만 '맥락 → 변화' 형식으로 적으세요.\n` +
        `9. 승인 목록 밖의 개인정보는 추측하지 마세요.`

...
describe('P0 기억 추출 — 카테고리 확장', () => {
  ...
  it('한 질문에서 여러 카테고리를 동시에 추출한다', () => {
    const found = extractByRules('무릎이 안 좋아서 오래 못 걷고, 예산도 2만원 이하예요');
    expect(found.length).toBeGreaterThanOrEqual(2);
  });

  it('저장 카드의 제목·태그가 하드코딩이 아니라 추출값에서 나온다', () => {
    const store = new ProposalStore();
    const proposal = store.create('u1', 'pr', '당뇨가 있어요', [], NOW);
  ...
  it('LLM 추출 결과가 규칙 결과보다 우선 병합된다', () => {
    const store = new ProposalStore();
    const proposal = store.create('u1', 'pr', '무릎이 안 좋아요', [], NOW);
    const llm = [{
      label: '이동 접근성', title: '이동 접근성', category: 'constraint' as const,
      content: 'LLM이 뽑은 값', tags: ['이동'], privacyLevel: 'sensitive' as const,
      semanticGroup: '이동',
    }];
    const candidates = store.extractMemories(proposal, llm);

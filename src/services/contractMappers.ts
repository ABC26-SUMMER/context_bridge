...
const exampleQuestions = {
  university_student: ["이번 방학에 뭐 공부해야 해?", "공기업 전산직 준비하려면 뭐부터 해야 해?"],
  older_adult: ["내일 딸이랑 어디 가면 좋아?", "키오스크 쓰는 법 쉽게 알려줘"],
  custom: ["내 상황에 맞는 방법을 알려줘", "어디서부터 시작하면 좋을까?"],
} as const;
...
export function getInitialApprovals(evaluations: EvaluatedContext[]) {
  return Object.fromEntries(
    evaluations.map((evaluation) => [
      ...
      evaluation.suggested && evaluation.valueVisible && !evaluation.exclusionReason,
    ]),
  );
}
...
function mapProposalIntent(proposal: ProposalResponse): DetectedIntent {
  return {
    ...
    label: proposal.selectionMode === "rules" ? "규칙 기반 맥락 선별" : "AI 기반 맥락 선별",

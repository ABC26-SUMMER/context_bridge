...
const configuredBaseUrl = publicEnv.apiBaseUrl;
...
export async function apiRequest<T>(path: string, options: ApiRequestOptions): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(configuredBaseUrl
      ? "API 서버에 연결할 수 없습니다. VITE_API_BASE_URL과 백엔드 실행 주소를 확인해 주세요."
      : "통합 API 서버에 연결할 수 없습니다. npm run dev 실행 상태를 확인해 주세요.");
  }

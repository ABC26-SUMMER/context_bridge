...
3. 승인 → `POST /api/proposals/{id}/answers` (승인한 카드 **ID만** 전송) → 답변 + 기억 후보
4. 기억 저장 → `POST /api/memory-candidates/{id}` (`{action:'save'|'ignore'}`)

## 프론트가 지켜야 할 4가지 (보안·계약)

1. **카드 값을 서버로 되보내지 않는다.** 답변 생성 땐 `approvedContextIds`(ID 배열)만.
...
- 답변 API: `/api/proposals/{id}/answers`
- 승인 ID 키: `approvedContextIds`
- 일회성 메모 키: `temporaryNote`
- 프로필·카드·감사 로그는 Controller API 경유
- Supabase 직접 연결은 로그인·세션 처리에만 사용

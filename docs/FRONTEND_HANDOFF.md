...
Mock 서버는 Supabase JWT의 사용자 유형을 판별해 대학생/고령층 fixture를 선택합니다. 질문의 핵심 단어로 관련 카드를 선별하고, proposal별 질문과 실제 `approvedIds`를 메모리에 보관해 생성 답변·승인 개수·감사 로그가 같은 요청을 가리키도록 구성했습니다. 실제 백엔드가 완성되면 `VITE_API_BASE_URL`만 교체합니다.

## 세션 2: Supabase Auth/Profile

상태: 구현 및 로컬 검증 완료. 커밋 `cb12211 feat: Supabase 인증 및 프로필 연동`
...
상태: 구현 및 로컬 검증 완료. 커밋 `3d31421 feat: 고령층 접근성 및 음성 기능 개선`
...
- 브라우저 Web Speech API 기반 한국어 연속 음성 질문 입력
...
음성 기능은 Chrome 계열 등 Web Speech API를 제공하는 브라우저에서 동작합니다. 사용자가 마이크 버튼을 누르면 입력을 시작하고 같은 버튼을 다시 누를 때 종료합니다. 중간에 멈추거나 여러 문장으로 나눠 말해도 기존 문장을 덮어쓰지 않고 누적하며, 브라우저가 침묵으로 인식 세션을 종료하면 사용자 종료 전까지 자동 재시작합니다. 마이크 권한이 없거나 API를 지원하지 않는 환경에서는 텍스트 입력을 유지하고 안내 문구를 표시합니다. 답변은 자동 재생하지 않으며 사용자가 `답변 읽기`를 눌렀을 때만 낭독합니다.
...
실제 백엔드가 준비되기 전에는 계약 타입이나 원본 Mock fixture를 임의 변경하지 않습니다. 동적 동작은 `contracts/mock-engine.mjs`에 분리합니다. `supabase_schema.sql`은 과거 데모 스키마이므로 팀 공용 Supabase에 실행하지 않습니다.

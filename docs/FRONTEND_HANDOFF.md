...
> v20 갱신: 아래 세션 기록 중 “프론트가 Supabase 테이블을 직접 조회·수정”하는
> 방식은 폐기했습니다. 현재 프론트는 Supabase Auth만 직접 사용하고, 프로필·카드·
> 감사 로그는 모두 Controller API를 경유합니다. 답변 경로는
> `/api/proposals/:proposalId/answers`, 요청 키는 `approvedContextIds`입니다.
...
- `/api/bootstrap`, `/api/proposals`, `/api/proposals/:proposalId/answers` 연결
...
- 테스트 계정 정보는 저장소에 기록하지 않습니다. 팀에서 별도로 전달받은 계정이 `invalid_credentials`를 반환하면 Supabase 계정 상태와 비밀번호를 확인해야 합니다.
